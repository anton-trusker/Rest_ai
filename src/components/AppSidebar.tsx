  ...
  return <>
      ...
      <aside className="hidden lg:flex fixed top-0 left-0 h-full w-64 bg-sidebar z-50 border-r border-sidebar-border flex-col">
        ...
        <nav className="flex-1 p-3 space-y-0.5 overflow-y-auto">
          {nav.map(item => <button key={item.path} onClick={() => navigate(item.path)} className={`nav-item w-full ${location.pathname === item.path || item.path !== '/dashboard' && location.pathname.startsWith(item.path) ? 'active' : ''}`}>
