...
## 5. Super Admin Dashboard (SaaS Control)
Only visible to `super_admin` role.

### 5.1 Client Management
- **Client List**: Table of all registered businesses.
- **Module Control**: Toggle features per client (AI, Syrve Pro, Multi-Location).
- **Usage Metrics**: Sync counts, AI requests, active users.

### 5.2 Feature Flags
- **Global Flags**: Turn off features globally for maintenance.
- **Canary Releases**: Enable features for specific `business_id`s.
